...
public class InputValidation {
    ...
    public static int optionsValidation(int iYear, int iMonth, int iDate) {
        ...
        if ( (isInt(iYear)) && iYear > 1900 && iYear < 2100 ) {
            ...
            throw new CustomException("BAD REQUEST ERROR 400: Year is invalid");
        }

        if ( (isInt(iMonth)) && iMonth > 0 && iMonth < 13 ) {
            sum  += 1;
        } else {
            throw new CustomException("BAD REQUEST ERROR 400: Month is invalid");
        }
        ...
        if (iMonth == 2) {
            if ((isInt(iDate)) && iDate > 0 && iDate < 30) {
                ...
                throw new CustomException("BAD REQUEST ERROR 400: Date is invalid");
            }
        }
